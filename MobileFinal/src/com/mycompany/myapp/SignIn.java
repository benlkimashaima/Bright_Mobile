...
import com.mycompany.myapp.gui.GestionSante;
import com.mycompany.myapp.gui.Gsante;
...
import userclassesM.StateMachineA;
import userclassese.StateMachinee;
//import userclassesM.StateMachineM;
...
Image sign = themeee.getImage("chboxv1.png");
        ...
        Toolbar.setGlobalToolbar(true);
          loginForm.getToolbar().addMaterialCommandToSideMenu("",FontImage.MATERIAL_IMPORTANT_DEVICES, e -> {});

  loginForm.getToolbar().addMaterialCommandToSideMenu("Santer",FontImage.MATERIAL_IMPORTANT_DEVICES, e -> {});
   ...
   loginForm.getToolbar().addMaterialCommandToSideMenu("Exit", FontImage.MATERIAL_EXIT_TO_APP, e -> {Display.getInstance().exitApplication();}); 
     ...
     loginForm.getToolbar().addMaterialCommandToSideMenu("TIME",FontImage.MATERIAL_TIMER, ev->{
        //s.addActionListener((e)->{
           
          Form hi = new Form("TIME");
        AnalogClock clock = new AnalogClock();
        hi.setLayout(new BorderLayout());
        hi.addComponent(BorderLayout.CENTER, clock);
        clock.start();
       hi.getToolbar().addMaterialCommandToLeftBar("", FontImage.MATERIAL_ARROW_BACK, e->{loginForm.show();});
        hi.show();
   
        });
 
     
      loginForm.getToolbar().addMaterialCommandToSideMenu("Notre Localisation",FontImage.MATERIAL_GPS_FIXED, ev->{

         if(current != null) {
            current.show();
            return;
        }
        new StateMachineA("/theme5");  
        });
            loginForm.getToolbar().addMaterialCommandToSideMenu("To do App",FontImage.MATERIAL_DONE, ev->{

         if(current != null) {
            current.show();
            return;
        }
        new StateMachinee("/theme0");  
        });

        Button signIn = new Button(" Visiter Notre Espace ",sign);
