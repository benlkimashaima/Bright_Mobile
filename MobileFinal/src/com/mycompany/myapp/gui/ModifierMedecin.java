 ...
 * @author User
 */
public class ModifierMedecin extends Form{
    
              private Resources themeee;
Form current;
     public ModifierMedecin(Form p){
                                      themeee = UIManager.initFirstTheme("/themeee");

        setTitle("Modifier un Medecin");
      ...
      getToolbar().addMaterialCommandToSideMenu("Affichage", FontImage.MATERIAL_LIST, e -> { new Lme(p).show(); });
   ...
   getToolbar().addMaterialCommandToSideMenu("Exit", FontImage.MATERIAL_EXIT_TO_APP, e -> {Display.getInstance().exitApplication();}); 
        ...
        Button btnValider = new Button("Modifier Medecin");
    ...
    dlg.add(blueLabel);
    TextArea ta = new TextArea("Vous voulez vraiment Modifier ce Medecin");
...
btnValider.addActionListener(e -> {
  ...
  ToastBar.Status status = ToastBar.getInstance().createStatus();
            status.setMessage(" Medecin Modifier ...");
...
////////////////////////////////////////////////////
       ok.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                if ((tfid.getText().length()==0)||(tfcoff.getText().length()==0)||(tfn.getText().length()==0)||(tfp.getText().length()==0)||(tft.getText().length()==0)||(tfe.getText().length()==0))
                    Dialog.show("Alert", "Please fill all the fields", new Command("OK"));
                else
                {
                    try {
                        Medecin t = new Medecin(Integer.parseInt(tfid.getText()), Integer.parseInt(tfcoff.getText()),tfn.getText(),tfp.getText(),tft.getText(),tfe.getText());
        ...
        addAll(tfid,tfcoff,tfn,tfp,tft,tfe,btnValider);
        getToolbar().addMaterialCommandToLeftBar("", FontImage.MATERIAL_ARROW_BACK
                , e-> p.showBack()); // Revenir vers l'interface précédente
