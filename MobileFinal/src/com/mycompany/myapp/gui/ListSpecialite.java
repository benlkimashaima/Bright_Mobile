...
getToolbar().addCommandToRightBar("", searchIcon, (e) -> {
   ...
   getToolbar().addMaterialCommandToSideMenu("Mail", FontImage.MATERIAL_INFO, e -> {
         Message m = new Message("Cher Docteur"
                 + "Vous étes invité à une consultation d'urgence"
                 + "Cordialement  ");
        
         
            Display.getInstance().sendMessage(new String[] {"mahmoudbenayed8@gmail.com"}, "rendez-vous URGENT ", m);
   ...
   getToolbar().addMaterialCommandToSideMenu("Exit", FontImage.MATERIAL_EXIT_TO_APP, e -> {Display.getInstance().exitApplication();}); 
     ...
     SpanLabel sp = new SpanLabel();
       ...
       // add(sp);
        ...
        getToolbar().addMaterialCommandToLeftBar("", FontImage.MATERIAL_ARROW_BACK, e-> { new UserForm().show();});
    }
